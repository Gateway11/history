...
#define ALOGI(...) __android_log_print(ANDROID_LOG_INFO, tag_, __VA_ARGS__)
	...
	void do_poll() {
		...
		while (true) {
			if (!speech_->poll(result))
				break;
            ALOGI("result : id \t %d \t type \t %d \t err \t %d", result.id, result.type, result.err);
...
static void com_rokid_speech_SpeechOptions_native_set_vad_mode(JNIEnv* env,
	...
	if (strcmp(sv, "cloud") == 0)
		...
		mode = VadMode::LOCAL;
	p->options->set_vad_mode(mode, 500);
	env->ReleaseStringUTFChars(v, sv);
}
